	...
	<main class="container py-5 flex-grow-1">
		<div class="d-flex justify-content-between align-items-center">
			...
			<c:if test="${sessionScope.loginMember.mRole eq 1}">
				<a class="btn btn-warning fw-bold rounded-1" href="<c:url value='/notice/create'/>">
					게시글 작성
				</a>
			</c:if>
		</div>
		<div class="d-flex justify-content-between align-items-center mb-4">
			...
			<form action="<c:url value='/notice/readSome'/>" method="get" class="d-flex align-items-center me-3">
				<select name="noticeSearchType" class="form-select me-2 w-auto pe-5">
					<option value="nTitle">제목</option>
					<option value="nContent">내용</option>
					<option value="mNickName">작성자</option>
				</select> 
				<input type="text" name="noticeSearchKeyword"
					class="form-control me-2 flex-grow-1"
					value="${pageMaker.cri.noticeSearchKeyword}"
					placeholder="검색어를 입력하세요" /> 
				<input class="btn btn-secondary" type="submit" value="조회" />
			</form>
					...
					<tbody>
						<c:forEach items="${list}" var="item">
							<tr>
								<td>${item.nNum}</td>
								<td>
									<a href="<c:url value='/notice/readOne?nNum=${item.nNum}'/>">${item.nTitle}</a>
								</td>
				...
				<ul class="pagination justify-content-center">
					...
					<li class="page-item ${paging.currentPage == 1 ? 'disabled' : ''}">
						<c:choose>
							...
							<c:otherwise>
								<a class="page-link" href="<c:url value='/notice/readSome'>
