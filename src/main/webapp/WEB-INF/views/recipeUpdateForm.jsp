...
<body>
						...
						<div class="row g-2">
							...
							<div class="col-sm-6 col-md-3">
								...
								<select class="form-select" id="rCookType" name="rCookType"
									...
									<option value="중식"
										${recipe.rCookType eq '중식' ? 'selected' : ''}>중식</option>
									<option value="양식"
									...
									<option value="디저트"
										${recipe.rCookType eq '디저트' ? 'selected' : ''}>디저트</option>
									<option value="음료"
										${recipe.rCookType eq '음료' ? 'selected' : ''}>음료</option>
									<option value="기타"
										${recipe.rCookType eq '기타' ? 'selected' : ''}>기타</option>
								</select>
							</div>
							...
							<div class="col-sm-6 col-md-3">
								<label for="rCookTime" class="form-label fw-bold small">요리
									...
									<option value="30분이내"
										${recipe.rCookTime eq '30분이내' ? 'selected' : ''}>30분이내</option>
									<option value="1시간이내"
										${recipe.rCookTime eq '1시간이내' ? 'selected' : ''}>1시간이내</option>
									<option value="1시간30분이내"
										${recipe.rCookTime eq '1시간30분이내' ? 'selected' : ''}>1시간30분이내</option>
									<option value="2시간이내"
										${recipe.rCookTime eq '2시간이내' ? 'selected' : ''}>2시간이내</option>
									<option value="2시간이상"
										${recipe.rCookTime eq '2시간이상' ? 'selected' : ''}>2시간이상</option>
								</select>
							</div>
							...
							<div class="col-sm-6 col-md-3">
								...
								<select class="form-select" id="rServing" name="rServings"
									...
									<option value="2인분"
										${recipe.rServings eq '2인분' ? 'selected' : ''}>2인분</option>
									<option value="3인분"
										${recipe.rServings eq '3인분' ? 'selected' : ''}>3인분</option>
									<option value="4인분"
										${recipe.rServings eq '4인분' ? 'selected' : ''}>4인분</option>
									<option value="5인분이상"
										${recipe.rServings eq '5인분이상' ? 'selected' : ''}>5인분이상</option>
								</select>
							</div>
							...
							<div class="col-sm-6 col-md-3">
								...
								<select class="form-select" id="rLevel" name="rLevel" required>
									<option value="쉬움" ${recipe.rLevel eq '쉬움' ? 'selected' : ''}>쉬움</option>
									<option value="보통" ${recipe.rLevel eq '보통' ? 'selected' : ''}>보통</option>
									<option value="어려움" ${recipe.rLevel eq '어려움' ? 'selected' : ''}>어려움</option>
								</select>
							</div>
						</div>
					</div>
			...
			<section class="mb-5">
				...
				<div class="card p-3">
					<textarea class="form-control" id="rIngredients"
						...
						placeholder="주재료, 양념재료, 소스재료등 따로 구분해서 작성 해주세요!">${recipe.rIngredients}</textarea>
				</div>
			</section>
						...
						<div class="step-block card p-4 shadow-sm mb-3 border">
							<div
								...
								<input type="hidden" name="cookingSteps[${status.index}].cNum"
									value="${step.cNum}"> <input type="hidden"
							...
							<div class="row g-3">
								<div class="col-lg-7">
									<label for="cookingSteps[${status.index}].cInstructions"
										class="form-label small text-muted">요리 내용</label>
									<textarea class="form-control"
										id="cookingSteps[${status.index}].cInstructions"
										name="cookingSteps[${status.index}].cInstructions" rows="5"
										placeholder="이 단계의 요리 순서를 자세히 적어주세요." required>${step.cInstructions}</textarea>
								</div>
										...
										<div
											...
											<c:if test="${not empty step.cImage}">
												<img class="stepImagePreview image-cover d-block"
													...
													alt="기존 스텝 이미지">
												<span class="stepImagePlaceholder d-none">이미지 미리보기</span>
											</c:if>
										</div>
	...
	<script src="<c:url value='/resources/js/recipeUpdateForm.js'/>"></script>
</body>
</html>