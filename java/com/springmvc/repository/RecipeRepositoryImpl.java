...
public class RecipeRepositoryImpl implements RecipeRepository {
	...
	public Recipe readOne(Integer rNum) {
		...
		String SQL = "SELECT r.*, m.m_nick_name, COALESCE(l.cnt, 0) AS like_count " +
		        "FROM recipe r " +
	...
	public List<Recipe> findPageOrderByView(int offset, int pageSize) {
		...
		String SQL = "SELECT r.*, COALESCE(l.cnt, 0) AS like_count " +
				  "FROM recipe r " +
				  "LEFT JOIN ( " +
				  "  SELECT r_num, COUNT(*) AS cnt " +
				  "  FROM recipe_like " +
	...
	//레시피 좋아요순 페이징
	@Override
	public List<Recipe> findPageOrderByLike(int offset, int pageSize) {
	    ...
	    String SQL =
	        ...
	        "  GROUP BY r_num " +
	        ") l ON l.r_num = r.r_num " +
	        "ORDER BY like_count DESC, r.r_num DESC " +
	        "LIMIT ? OFFSET ?";
