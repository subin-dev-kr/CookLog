...
public class WelcomeController {
	...
	public String recipeList(@RequestParam(value = "page", defaultValue = "1") int page, 
	    ...
	    if ("view".equals(sort)) {// 조회순
	        recipeList = recipeService.getRecipePageByView(paging.getOffset(), paging.getPageSize());
	    } else if("like".equals(sort)) { //좋아요순
	    	recipeList = recipeService.getRecipePageByLike(paging.getOffset(), paging.getPageSize());
	    } else { //최신순
	        recipeList = recipeService.getRecipePage(paging.getOffset(), paging.getPageSize());
	    }
	    // JSP로 전달
